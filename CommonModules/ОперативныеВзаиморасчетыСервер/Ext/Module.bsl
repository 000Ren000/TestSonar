			...
			Если СтрокаЗачета.ЗачетАванса Тогда
				...
				Если НЕ ДвойнаяЗаписьНеНужна Тогда
					...
					#Область ПриходДолг
						...
						НовСтр_Долг_Приход.АналитикаУчетаПоПартнерамПриемник  = Справочники.КлючиАналитикиУчетаПоПартнерам.ПустаяСсылка();
						НовСтр_Долг_Приход.ОбъектРасчетовПриемник             = Справочники.ОбъектыРасчетов.ПустаяСсылка();
						НовСтр_Долг_Приход.ВалютаПриемник             = Справочники.Валюты.ПустаяСсылка();
						НовСтр_Долг_Приход.СуммаПриемник                      = 0;
...
Процедура ДополнитьТаблицуИзмененийПриемниками(ТаблицаИзменений, Параметры, РассчитываемыеИзмерения = Неопределено)
	...
	Если ОбработчикОбновленияВыполнен(Обработчики) Тогда
		...
		|	ИЗ
		|		РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК РасчетыСКлиентамиПоСрокам
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаИзмененийСПриемниками КАК Изменения
		...
		Если СтрокаРасчетов.ПорядокОперации >= ГлобальныеПеременные.ПорядокФакт Тогда
			...
			Если ТипИсточника = "Оплата" Тогда
				...
				НовСтр.ПорядокЗачета = ?(СтрокаЗачета.ДатаПлановогоПогашенияОплаты = Дата(1,1,1),
													...
													Формат(СтрокаЗачета.ДатаПлановогоПогашенияОплаты, "ДФ=yyyyMMdd")
														+ Сред(СтрокаРасчетов.ПорядокЗачетаПоДатеПлатежа, 9, 1)
														+ "000000"
														+ Прав(СтрокаРасчетов.ПорядокЗачетаПоДатеПлатежа,13));
