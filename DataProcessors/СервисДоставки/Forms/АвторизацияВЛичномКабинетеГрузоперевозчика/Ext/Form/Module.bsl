...
Процедура УстановитьОформлениеПунктовМенюКурьерика(СостояниеПодключения = Неопределено)
	...
	Иначе
		...
		Элементы.ГиперссылкаВыборСклада.Подсказка = НСтр(
			"ru = 'С выбранного склада курьеры будут забирать заказы. "
			+ "Дальнейшая настройка пунктов выдачи заказов производится "
			+ "в личном кабинете логиста (после получения доступа к сервису).'");
